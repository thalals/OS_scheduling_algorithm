...
public class SRT {
    ...
    //나중에 간트차트로 사용할 수 있을까?
    //프로세스 리스트 삽입
    public void insert(ArrayList<Process> p, int timeslice, StackedGanttChart demo) {
    	for(Process job : p) {
    		job.Wait_time=0;
    		job.Response_time=-1;
    		job.Return_time=0;
        ...
        //runningThread.execute(() -> {//스레드를 실행하겠다.-주석처리.
        	...
        	if(cpuCount==p.time_Remain) {
            	...
            	System.out.println(p.ID+" 프로세스 응답 시간: "+p.Response_time+
            			"/ 대기 시간: "+p.Wait_time+
            			"/ 반환 시간: "+p.Return_time);
            	demo.createDatasetComplete(cpuCount, p.uID, 5);
        	}
        	else{
        		p.time_Remain=p.time_Remain-timeslice;
        		demo.createDatasetC_Ready(cpuCount, p.uID, 5);
				readyQueue.add(p);
        	}
        	ReadyQueueChange(GetSRTIndex(),demo);
    }
    ...
    void ReadyQueueChange(int index, StackedGanttChart demo) {//레디큐-cpu 사이 제어 함수
    	if(cpuUse==false&&readyQueue.size()!=0) {
    		if(readyQueue.get(index).Response_time==-1){
    			...
    			//고러면 최초 응답시간을 만들어줘야죠
    			demo.createDatasetR_Ceady(timeLapse-readyQueue.get(index).Arrival_time, readyQueue.get(index).uID, 5);
    		}
    		else demo.createDatasetR_Ceady(readyQueue.get(index).waiting, readyQueue.get(index).uID, 5);
    		start(readyQueue.get(index),index,demo);//start 함수는 인접한 레디큐 제어 함수에서만 관리하도록 한다.
    	}
    	else
    		QueueJob(demo);
    }
    int GetSRTIndex() {//가장 짧은 서비스시간을 가지는 프로세스의 인덱스를 찾아주는 함수
    	int minSize=0;
    	for(int i=0;i<readyQueue.size();i++) {
    ...
    void QueueJob(StackedGanttChart demo) {//레디큐-준비큐 사이 제어 함수
    	...
    	if(Queue.size()!=0&&Queue.get(0).Arrival_time<=timeLapse) {//레디큐에 도착한 경우
    		...
    		demo.createDatasetReady(Queue.get(0).Arrival_time, Queue.get(0).uID, 5);
    		Queue.remove(0);
    		ReadyQueueChange(GetSRTIndex(),demo);
    	}
