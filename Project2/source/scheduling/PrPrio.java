...
public class PrPrio {
	...
	private boolean newProcess=false;//새로운 프로세스가 대기큐에 들어왔는지 확인
    	...
    	readyQueue.remove(index);//cpu에서 사용하는 프로세스는 레디큐에서 삭제
        	while(cpuCount!=p.time_Remain) {//서비스 시간을 충족할 때까지 수행
        		...
        		if(newProcess==true) {
        			if(readyQueue.size()==0) 
        				newProcess=false;
        			else if (cpuCount!=p.time_Remain && p.Priority_Number>readyQueue.get(readyQueue.size()-1).Priority_Number) {
        				newProcess=false;
        				demo.createDatasetC_Ready(cpuCount, p.uID, 3);
        	...
        	if(cpuCount==p.time_Remain) {
            	...
            	System.out.println(p.ID+" 프로세스 응답 시간: "+p.Response_time+
            			"/ 대기 시간: "+p.Wait_time+
            			"/ 반환 시간: "+p.Return_time);
            	demo.createDatasetComplete(cpuCount, p.uID, 3);
            	ReadyQueueChange(GetPriIndex(),demo);
        	}
        	else{
				readyQueue.add(p);
				ReadyQueueChange(GetPriIndex(),demo);
    ...
    int GetPriIndex() {//가장 짧은 우선순위를 가지는 프로세스의 인덱스를 찾아주는 함수
    	int minSize=0;
    ...
    void QueueJob(StackedGanttChart demo) {//레디큐-준비큐 사이 제어 함수
    	...
    	if(Queue.size()!=0&&Queue.get(0).Arrival_time<=timeLapse) {//레디큐에 도착한 경우
    		...
    		newProcess=true;
    		demo.createDatasetReady(Queue.get(0).Arrival_time, Queue.get(0).uID, 3);
    		Queue.remove(0);
    		ReadyQueueChange(GetPriIndex(),demo);
    	}
